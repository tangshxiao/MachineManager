...
export default {
  ...
  // 【修改点2】：新增 watch 监听器，替代原本的 @input 事件
  watch: {
  ...
  methods: {
    ...
    handleDeviceInputFocus() {
      // 【修改点3】：延迟 150ms 展开列表，防止视图突然更新打断安卓端刚刚弹起的输入法
      setTimeout(() => {
    ...
    // 【修改点4】：删除了原本强行截断文字的 onDeviceSearch 方法，不再使用
    ...
    handleDeviceSearch() {
      ...
      // 【修改点5】：将防抖时间提升到 800ms，给用户敲击中文拼音留出足够的时间，防止边打字边搜索导致卡顿
    ...
    async searchDevices() {
      ...
      try {
        // 【修改点6】：在请求接口前，过滤掉奇怪的符号，保留中英文和数字传给后端，这样既安全又不影响前端输入体验
    ...
    async submit() {
      ...
      try {
        ...
        try {
          ...
          if (!imgs) console.warn('图片上传失败，将在离线数据中保存图片路径');
        } catch (imgError) {
